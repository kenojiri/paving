resource "aws_route53_record" "wildcard-sys" {
  ...
  name = "*.sys.${var.environment_name}.${aws_route53_zone.zone[0].name}"
...
resource "aws_route53_record" "wildcard-apps" {
  count = var.cloudflare_api_key == "" ? 1 : 0
  name = "*.apps.${var.environment_name}.${aws_route53_zone.zone[0].name}"
  zone_id = aws_route53_zone.zone[0].zone_id
  type    = "A"
  alias {
    name = aws_lb.tas.dns_name
...
resource "aws_route53_record" "ssh" {
  count = var.cloudflare_api_key == "" ? 1 : 0
  name = "ssh.sys.${var.environment_name}.${aws_route53_zone.zone[0].name}"
  zone_id = aws_route53_zone.zone[0].zone_id
  type    = "A"
  alias {
    name = aws_lb.tas.dns_name
...
resource "aws_route53_record" "wildcard-login-sys" {
  count = var.cloudflare_api_key == "" ? 1 : 0
  name = "*.login.sys.${var.environment_name}.${aws_route53_zone.zone[0].name}"
  zone_id = aws_route53_zone.zone[0].zone_id
  type    = "A"
  alias {
    name = aws_lb.tas.dns_name
...
resource "aws_route53_record" "wildcard-uaa-sys" {
  count = var.cloudflare_api_key == "" ? 1 : 0
  name = "*.uaa.sys.${var.environment_name}.${aws_route53_zone.zone[0].name}"
  zone_id = aws_route53_zone.zone[0].zone_id
  type    = "A"
  alias {
    name = aws_lb.tas.dns_name
...
resource "cloudflare_record" "wildcard-login-sys" {
  count = var.cloudflare_api_key == "" ? 0 : 1
  zone_id = var.cloudflare_zone_id
  name = "*.login.sys.${var.environment_name}"
  value = aws_lb.tas.dns_name
  type = "CNAME"
  ttl = 60
  proxied = false
...
resource "cloudflare_record" "wildcard-uaa-sys" {
  count = var.cloudflare_api_key == "" ? 0 : 1
  zone_id = var.cloudflare_zone_id
  name = "*.uaa.sys.${var.environment_name}"
  value = aws_lb.tas.dns_name
  type = "CNAME"
  ttl = 60
  proxied = false
